...
math.powの意味だが、'指数部分は(bandit_dict[bnd]['win'] + bandit_dict[bnd]['loss'] + bandit_dict[bnd]['opp'])'で、今まで引かれた回数を指している。
...
| op_continueなし | 名前 | 結果 |
|----|----|----|
|0.5|version36|1000→1030|
|1.0|version37|1100→1113|
|0.8|version38|1030→1026|
|0.9|version39|1000→1000|
|1.1|version40|1030→1043|

### 1/20
***

最近思ったのは、終盤戦では、すべての自動販売機の当たり確率が均質化されて、ほぼrandomに選んでも性能はそこまで変化しないのではないか。そうだとしたら、op_continueは、特に終盤に関しては、何も意味がなかったことになる。つまり、op_continueに関する仮説が大外れだったことになる。(時間返せ)　スコア殆ど変わらないし。

これを踏まえた上で、べき乗の底を変化させる。この次は1.0の周りでlossペナルティを振動させる実験?とにかく、エージェント自体の大幅改造はそこまでうまく行かなそう。

| op_continueなし ペナ1.10固定 | 名前 | 結果 |
|----|----|----|
|0.962|version46|1101|
|0.963|version41|1050|
|0.964|version42|860|
|0.965|version40|1026|
|0.966|version43|975|
|0.967|version44|990|
### 1/21
***
| op_continueなし ペナ1.00固定 | 名前 | 結果 |
|----|----|----|
|0.962|version47|1005|
|0.963|version48|960|
|0.964|version49|993|
|0.965|version37|950|
|0.966|version50|966|

ペナ1.1のほうが強かった？？いや、それよりも、パラメータの至るところに極大点があるようだ。極大点を探し当てられたら、良いスコアが出る。ということは実験ゲー???

他の人がスコアを上げていて、圏外に吹き飛ばされそう、悲しみ。
### 1/22
***

Notebookをいじっていたら、たまたま強そうな手に遭遇した。分母の当たり回数カウントの係数を小さくするというものだ。テストでは前半に強かった。自分が当たりを引くにつれて信頼度が増加する。

| op_continueなし ペナ1.00固定 | 名前 | 結果 |
|----|----|----|
|0.3(1/21)|version51|1010|
|0.4|version52|974|
|0.5|version53|900|
|0.6|version54|940|
|0.7|version55|1120|
...
### 1/23
***
はあ、sub直後はいい感じだったのに時間が立つと銀圏から外れた...悲しみ
とりあえず0.07当たりで振動
...
### 1/26
***
今までの再提出(サブは無駄にしない)だが、新しいのも少し試す。
ver40~42+ver62,63
...
### 1/27
***
銅メダルはいけそう。もう一捻り加えて、もう少し食らいつきたい。完全にランダムな項を付け加える。
ベースは(55+α)と46で、２日かけて実験を行う。

| 振幅 (ペナ=1.1,分母係数=0.7) | 名前 | 結果 |
|----|----|----|
|0.1|version64||
|0.2|version65||
|0.3|version66||
...
### 1/28
***
原点回帰で、0.965ベースにlossペナルティを変化させるのをもう一度、前回と違ってopp_continueはない。0.965とペナ以外は何も変わらない。
...
効果があるか微妙だが、最初の引きがdescendingなものも試す。どうなる???
...
銀圏に届かないよ（泣）、悲しみ
いい線いってそうだが、ちょっと力不足感が否めない。安全圏に2つエージェントがいるから銅は多分取れると思うが...銀は遠いなあ
仕方がないので、分子係数:0.7、ペナ:variable、0.965にしてみる。
...
79が強そう。脳死でこれだけ提出してたい...
Greedy Desision Treeを使おうとも考えたが、どうやらpyファイルでなく.tar.gzファイルで提出しないとうまく行かないらしいので、諦める。
...
### 2/2
***
終了、銅メダル取れるか取れないかの瀬戸際にいた、自分の力不足が悔やまれる。